...
export default function Tableinfo() {
    ...
    async function Handleapi() {
      ...
      const response1 = await axios.get('http://localhost:3000/Projects')
        ...
        const response = await axios.get('http://localhost:5000/rfile')
        .then(response => {
          setC(response.data)
        }
        )      
    ...
    let response2 = axios.get('http://localhost:5000/getdatatypes/' + c)
      .then((response2) =>
        setB(response2.data)
      )
            ...
            <Table stickyHeader size="small" aria-label="sticky table" className="tablerow1 td">
                <TableHead className='tablehead'>
                    <TableRow >
                        ...
                        <TableCell  style={{fontSize: "0.75rem",fontWeight: 600, color:"#656363"}}>Column</TableCell>
                        {/* <TableCell  style={{fontSize: "0.75rem",fontWeight: 600}}>Nonnull_count</TableCell> */}
                        <TableCell  style={{fontSize: "0.75rem",fontWeight: 600, color:"#656363",paddingLeft: "12px"}}>Datatype</TableCell>
                    </TableRow>
